...
class userControllers {
    ...
    try {
      ...
      if (hierarchy_id === 1 && actualUser.hierarchy_id !== 1) {
        ...
        return res.status(400).json({
          message:
            "Você não pode criar um usuário com cargo de hierarquia maior que o seu.",
        });
      }
  ...
  public async updateOther(req: Request, res: Response): Promise<Response> {
    ...
    try {
      ...
      const newUser: newUserUpdateSelf = {};

      // validating hierarchy_id and if user can do this changes
      if (hierarchy_id) {
        if (!Number(hierarchy_id) || hierarchy_id > 4 || hierarchy_id < 1)
          return res
            .status(400)
            .json({ message: "Cargo de hierarquia inválido." });

        if (!actualUser || !actualUser.hierarchy_id)
          return res
            .status(500)
            .json({ message: "O ocorreu um erro interno no servidor. FAKE" });

        if (actualUser.hierarchy_id >= user.hierarchy_id) {
          return res.status(403).json({
            message:
              "Você não tem permissão para alterar o cargo da hierarquia deste usuário.",
          });
        }
        ...
        if (hierarchy_id === 1 && actualUser.hierarchy_id !== 1) {
          return res.status(403).json({
            message:
              "Você não tem permissão para alterar o cargo da hierarquia deste usuário para um cargo maior que o seu.",
          });
        }
